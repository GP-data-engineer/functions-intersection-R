...
j <- 2
k <- 2
blk <- 0		#blokada zmiennej "k" do wyłączenia 2-go while
x1 <- function(x)
{		#x <<- round(seq(s,j, length.out=k), 3) #wartości X nie zaokrąlać bo długo szuka y-ów
		x <<- seq(s,j, length.out=k)} 
fy1 <- function(x)
{		x1(x)
		y1 <<- round(8*(x^2), 3)} 	#round(,3) daje zaokrąglenie do 3 miejsc po ','
fy2 <- function(x)
{		x1(x)
		y2 <<- round(64*x*(log(x)), 3)}
spr1 <- function(x)			#szukamy tylko 2-ch równych wartości z przedziału x[26,27]
{d1 <<- length(x)				#długość wektora x przypisana do zmiennej
for(i in 1:d1)	{			#długość iteracji przypisana do długości wektora x
	if(fy1(x[i]) == fy2(x[i]))	{
	v10 <<- x[i]
	h10 <<- fy1(x[i])
	p10 <<- i
	cat(" wykresy funkcji przecinają się w punkcie o współrzędnych: \n")
	cat(" Pozycja =",p10,"\n","X =",v10,"\n","Y =",h10,"\n")
	blk <<- 1				#blokada dla funkcji kp1 przed zwiększeniem k+1
	break	}
}}
kp1 <- function(b){			#=========k podniesione o 1=============
			if (b != 1){	#blokada gdyby znalały się dwie równe wartości w funkcjach
			k <<- k+1
			}}
while(fy1(x[j]) < fy2(x[j])) 	{	#====1 pętla while określa czy funkcje się przecinają========
					fy1(x)
					fy2(x)
					j <- j+1
					k <- j
...
s <- j-1 			#zmiana zakresu wartości x czyli od ostatniej (27-1=26) szukamy w przedziale x[26...27]
d4 <- length(x) 		#przypisanie do zmiennej długości wektora X
while(d4 == k) {		#====2 pętla while sprawdza gdzie funkcje dokładnie się przecinają======
		fy1(x)
		fy2(x)
		spr1(x)
		kp1(blk)
...
mtext("2 - Wykres z punktem przecięcia się funkcji:", adj=0.2, line = 1.235)
